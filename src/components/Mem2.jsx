...
const Mem2 = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (Array.isArray(res.data)) {
        const filtered = res.data.filter(t => t.assignedTo === "Team 2")
        setTasks(filtered)
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-emerald-800 from-indigo-900 via-gray-900 to-black text-white p-5">
      ...
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 p-8 rounded-2xl shadow-2xl w-[90%] max-w-md text-center">
        ...
        <div className="bg-white/10 p-4 rounded-lg mb-6">
          <p className="text-sm text-gray-200">Welcome Member 2 👋 (Team 2)</p>
        ...
        <button 
          ...
          className="w-full bg-purple-500 hover:bg-purple-600 transition py-2 rounded-lg font-semibold shadow-lg"
        >
          {showTasks ? "Hide Dashboard" : "Open Dashboard"}
        </button>
        ...
        {showTasks && (
          <div className="mt-6 text-left max-h-60 overflow-y-auto space-y-2">
            ...
            {tasks.length === 0 ? (
              ...
              tasks.map(t => (
                <div key={t.id} className="bg-white/10 p-2 rounded-lg text-sm border-l-4 border-purple-500">
                  {t.task}
                </div>
              ))
            )}
